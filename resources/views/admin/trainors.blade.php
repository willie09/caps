    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            <div class="relative">
                <button onclick="openAddTrainorModal()" class="bg-gray-300 hover:bg-gray-100 text-black px-4 py-2 mr-2 rounded-md text-sm font-medium">
                        Add Trainor
                    </button>
                <input type="text" id="searchTrainor" placeholder="Search Trainors..." 
                       class="w-48 px-4 py-1.5 border border-gray-300 dark:border-gray-600 rounded-md bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                                    ...
                                    <div class="p-6">
                                        ...
                                        <div class="flex justify-center mb-4">
                                            @if($trainor->profile_image && file_exists(public_path('storage/' . $trainor->profile_image)))
                                                <img src="{{ asset('storage/' . $trainor->profile_image) }}" alt="{{ $trainor->name }}" class="w-24 h-24 rounded-full object-cover border-2 border-gray-300 dark:border-gray-600">
                                            @else
                                                <div class="w-24 h-24 rounded-full bg-gradient-to-br from-blue-400 to-purple-600 flex items-center justify-center text-white text-2xl font-bold">
                                        ...
                                        <div class="text-center">
                                            <h3 class="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-1">{{ $trainor->name }}</h3>
                                            <p class="text-sm text-gray-600 dark:text-gray-400 mb-2">{{ $trainor->email }}</p>
                                            
                                            
                                            @if($trainor->phone)
                                                <p class="text-sm text-gray-600 dark:text-gray-400 mb-3">{{ $trainor->phone }}</p>
                                            @endif
                                        ...
                                        <!-- Action Buttons -->
                                        <div class="flex justify-center space-x-2 pt-4 px-2 border-t border-gray-200 dark:border-gray-700">
                                            <button onclick="openViewTrainorModal({{ $trainor }})" class="text-green-600 hover:text-green-900 dark:text-green-400 dark:hover:text-green-300 text-sm font-medium">
                                                View
                                            </button>
                                            <button onclick="openEditTrainorModal({{ $trainor }})" class="text-indigo-600 hover:text-indigo-900 dark:text-indigo-400 dark:hover:text-indigo-300 text-sm font-medium">
                                                Edit
                                            </button>
                                            <form action="{{ route('trainors.destroy', $trainor) }}" method="POST" class="inline">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="text-red-600 hover:text-red-900 dark:text-red-400 dark:hover:text-red-300 text-sm font-medium" onclick="return confirm('Are you sure you want to delete this trainor?')">
                                                    Delete
                                                </button>
                                            </form>
                                        </div>
