<x-app-layout>
    ...
    <div class="py-6">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg p-6">
                ...
                <button onclick="document.getElementById('inventoryModal').style.display='flex'" class="inline-flex items-center px-4 py-2 bg-indigo-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-indigo-300 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition ease-in-out duration-150">
                    Add Inventory
                </button>
            </div>
                        ...
                        <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                            ...
                            <tr>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-sm font-medium space-x-2">
                                    <button onclick="openInventoryEditModal({{ $inventory->id }}, '{{ $inventory->name }}', '{{ $inventory->date }}', {{ $inventory->quantity }}, {{ $inventory->amount }})" class="text-indigo-600 hover:text-indigo-900">Edit</button>
            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg p-6">
                <div class="flex justify-between items-center mb-4">
                    ...
                    <button onclick="document.getElementById('productModal').style.display='flex'" class="inline-flex items-center px-4 py-2 bg-indigo-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-indigo-300 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition ease-in-out duration-150">
                        Add Product
                    </button>
                </div>
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                        ...
                        <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                            ...
                            <tr>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-sm font-medium space-x-2">
                                    <button onclick="openProductEditModal({{ $product->id }}, '{{ $product->product_name }}', {{ $product->quantity }}, {{ $product->selling_price }}, {{ $product->amount_sold }})" class="text-indigo-600 hover:text-indigo-900">Edit</button>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-lg w-full max-w-md p-6">
            ...
            <form method="POST" action="{{ route('products.store') }}">
                ...
                <div class="flex justify-end space-x-2">
                    ...
                    <button type="submit" class="px-4 py-2 bg-indigo-600 text-white rounded-md hover:bg-indigo-400">Add</button>
                </div>
            </form>
        </div>
    </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-lg w-full max-w-md p-6">
            ...
            <form method="POST" id="inventoryEditForm">
                ...
                <div class="flex justify-end space-x-2">
                    ...
                    <button type="submit" class="px-4 py-2 bg-indigo-600 text-white rounded-md hover:bg-indigo-400">Update</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <div id="productEditModal" style="display:none;" class="fixed inset-0 flex items-center justify-center z-50 bg-black bg-opacity-50">
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-lg w-full max-w-md p-6">
            ...
            <form method="POST" id="productEditForm">
                ...
                <div class="flex justify-end space-x-2">
                    <button type="button" onclick="closeProductEditModal()" class="px-4 py-2 bg-gray-600 hover:bg-gray-400 dark:bg-gray-600 rounded-md text-gray-700 dark:text-gray-200">Cancel</button>
                    <button type="submit" class="px-4 py-2 bg-indigo-600 text-white rounded-md hover:bg-indigo-400">Update</button>
                </div>
            </form>
        </div>
    ...
    <script>
        ...
        function openProductEditModal(id, productName, quantity, sellingPrice, amountSold) {
